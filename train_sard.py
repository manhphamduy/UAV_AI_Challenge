...
from dataset_sard import SARDDataset
...
num_classes = 12  # Giữ nguyên 12 classes như VisDrone
batch_size = 2
...
# --- Đường dẫn ---
train_path = "data/sard/train"
val_path = "data/sard/valid"
best_model_path = "models/sard_best_model.pth"
checkpoint_path = "models/checkpoint_sard.pth"
pretrained_model = "models/img_best_model.pth"  # ĐƠN GIẢN: Chỉ cần best_model.pth
...
train_dataset = SARDDataset(train_path, transforms=transform)
val_dataset = SARDDataset(val_path, transforms=transform)
...
# Load TOÀN BỘ weights từ best_model.pth (bao gồm cả head)
if os.path.exists(pretrained_model):
    ...
    print(f"✅ Đã load TOÀN BỘ weights (backbone + head)")
    print(f"ℹ️  Số classes: {num_classes} (giữ nguyên)")
else:
    print(f"⚠️  Không tìm thấy {pretrained_model}, training from scratch với ImageNet weights")
